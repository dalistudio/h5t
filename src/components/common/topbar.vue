  ...
  <header class="top-container">
		<div class="top-bar clearfix">
			<div class="topbar-nav">
				<ul class="nav-wrap">
					<li v-for="(item,index) in navs" :key="item.id" class="nav clearfix">
						<template v-if="item.name === '小米商城'"><a :href="item.sourceUrl" class="nav-name">{{item.name}}</a></template>
            <template v-else><a :href="item.sourceUrl" class="nav-name" target="_blank">{{item.name}}</a></template>
			...
			<div class="topbar-cart fa cart-arrow-down" @mouseenter="evtCartEnter" @mouseleave="evtCartOut">
				<div class="cart" :class="{'active': cartStatus}">
					...
					<a href="http://static.mi.com/cart/">购物车(<span>0</span>)</a>
				</div>
				<div class="cart-list" transition="fadein" v-show="cartStatus"> 购物车中还没有商品，赶快选购吧！ </div>
			</div>
