	...
	<div class="top-banner clearfix">
		<div class="site-category" @mouseleave="evtSideLeave">
			<div class="top-side-left">
				<ul class="side-left">
					<li class="side-item" @mouseenter="evtSideEnter(item.type)" v-for="item in sideItems" :key="item.id">
						{{item.content}}
					</li>
				</ul>
			</div>
			<div class="site-category-detail" v-show="goodsStatus">
				<ul class="category-items" v-for="goods in filterCurrGoods" :key="goods.id">
					<li class="category-goods" v-for="item in goods" :key="item.id">
...
<script>
import slide from './topslide.vue'
export default {
	data () {
		return {
			index: 0,
